...
//使用bison的 -r all 选项 在生成的*.output文件开头： State XXX conflicts: XXX shift/reduce //找到相应的State 进一步分析即可 //查看发生冲突的 前看符号 //发生冲突的 产生式规则 一定可以从上一个状态中 **同时**推出 //通过改写文法，减少First_S集重复的可能性 //即 可能同时推出的产生式规则中 不同的中间符之后 可能的第一个Token        
